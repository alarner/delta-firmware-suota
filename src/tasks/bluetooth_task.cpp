...
static void adv_tim_cb(OS_TIMER timer)
{
        ...
        OS_TASK_NOTIFY(task, ADV_TMO_NOTIF, (eNotifyAction)OS_NOTIFY_SET_BITS);
}
...
static void alert_tim_cb(OS_TIMER timer)
{
        ...
        OS_TASK_NOTIFY(task, ALERT_TMO_NOTIF, (eNotifyAction)OS_NOTIFY_SET_BITS);
}
...
static void conn_params_timer_cb(OS_TIMER timer)
{
        ...
        if (conn_dev) {
                ...
                OS_TASK_NOTIFY(conn_dev->current_task, PXP_UPDATE_CONN_PARAM_NOTIF,
                                                                        (eNotifyAction)OS_NOTIFY_SET_BITS);
        }
}
...
static void bas_tim_cb(OS_TIMER timer)
{
        ...
        OS_TASK_NOTIFY(task, BAS_TMO_NOTIF, (eNotifyAction)OS_NOTIFY_SET_BITS);
}

void bleConnectionTask(void *taskParameters)
{
                        ...
                        if (conn_dev && conn_dev->expired) {
                                ...
                                if (param_connections) {
                                        OS_TASK_NOTIFY(OS_GET_CURRENT_TASK(),
                                                PXP_UPDATE_CONN_PARAM_NOTIF, (eNotifyAction)OS_NOTIFY_SET_BITS);
                                }
                        }
                }
        }
